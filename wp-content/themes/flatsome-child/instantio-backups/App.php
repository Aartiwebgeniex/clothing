...
class App {
	...
	public $layout_class = "";
	public $suggested_products = null;
	...
	public function ins_layout_three() {
		...
		if ( $this->layout == 2 || $this->layout == 3 ) :
			...
			<div class="ins-checkout-popup ins-checkout-modern <?php echo esc_attr( $ins_layout_class ) ?>">
				...
				<?php
			// Render suggested products outside the cart layout
			// Use custom theme function if available (for size-based/random logic)
			$suggested_products = function_exists('p2c_get_suggested_products') ? p2c_get_suggested_products() : ins_get_suggested_products();
			
			if ( $suggested_products && $suggested_products->have_posts() ) {
				// Use theme template if available
				$template = locate_template('instantio/templates/suggested-products.php');
				if ( ! $template ) {
					$template = INS_INC_PATH . '/templates/suggested-products.php';
				}
			...
			?>
				...
				<div class="ins-checkout-layout ins-checkout-layout-3 <?php echo esc_attr( $ins_layout_class ) ?>">
					<?php
					// Add toggle button for suggested products if they exist
					if ( $suggested_products && $suggested_products->have_posts() ) {
						?>
						<div class="ins-suggested-products-toggle-btn collapsed" onclick="toggleSuggestedProducts()">
							<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
								<path d="M10 12L6 8L10 4" stroke="#666" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
							</svg>
						</div>
						<?php
					}
