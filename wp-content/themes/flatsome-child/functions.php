...
/**
 * Remove version query string from CSS & JS files
 */
function p2c_remove_css_js_version( $src ) {
    if ( strpos( $src, 'ver=' ) ) {
        $src = remove_query_arg( 'ver', $src );
    }
    return $src;
}
add_filter( 'style_loader_src', 'p2c_remove_css_js_version', 10, 1 );
add_filter( 'script_loader_src', 'p2c_remove_css_js_version', 10, 1 );
...
function p2c_instantio_template_override($template_path) {
    ...
    // Check for cart-modern.php
    else if (strpos($template_path, 'cart-modern.php') !== false) {
        $theme_template = get_stylesheet_directory() . '/instantio/templates/cart-modern.php';
    }
...
add_filter('ins_cart_template', 'p2c_instantio_template_override', 10, 1);
...
/**
 * Get suggested products - simplified version
 *
...
function p2c_get_suggested_products() {
    ...
    // Collect IDs to exclude (items already in cart)
    foreach ( WC()->cart->get_cart() as $cart_item ) {
        $exclude_ids[] = $cart_item['product_id'];
        if ( !empty($cart_item['variation_id']) ) {
            $exclude_ids[] = $cart_item['variation_id'];
        }
    }

    // Return 4 random in-stock products
    $args = array(
...
function p2c_gallery_image_size($size) {
    ...
    if (has_term('bundle', 'product_type', $product->get_id())) {
        return 'full';
    }
    return $size;
}
...
/**
 * Add floating add to cart button for mobile on single product pages
 */
add_action('wp_footer', 'add_floating_add_to_cart_mobile', 100);

function add_floating_add_to_cart_mobile() {
    ...
    // Check if it's a bundle product (using product type or taxonomy)
    $is_bundle = $product_type === 'bundle' || has_term('bundle', 'product_type', $product_id);
    ...
    <div class="p2c-floating-cart" id="p2c-floating-cart">
        <div class="floating-cart-content">
            <div class="floating-price">
                <?php echo $product->get_price_html(); ?>
                <span class="selected-size" style="display: none;"></span>
            </div>

            <?php if ($product_type === 'simple'): ?>
                <div class="floating-qty">
                    <button type="button" class="qty-minus">-</button>
                    <input type="number" class="qty-input" value="1" min="1" max="<?php echo $product->get_max_purchase_quantity(); ?>">
                    <button type="button" class="qty-plus">+</button>
                </div>

                <button type="button" class="floating-add-to-cart" data-product-id="<?php echo $product_id; ?>">
                    Add to Cart
                </button>
            <?php elseif ($product_type === 'variable'): ?>
              <a href="#picker_pa_size" class="floating-select-options" role="button">

                    Select Options
                </a>
            <?php elseif ($is_bundle): ?>
                <a href="#bundle-summary" class="floating-build-bundle">
                    Build Your Bundle
                </a>
            <?php else: ?>
                <a href="#product-<?php echo $product_id; ?>" class="floating-add-to-cart">
                    View Product
                </a>
            <?php endif; ?>
        </div>
...
/**
 * Disable WooCommerce variation validation alert for floating cart
 */
add_filter('woocommerce_add_to_cart_validation', 'disable_variation_validation_alert', 10, 5);

function disable_variation_validation_alert($passed, $product_id, $quantity, $variation_id = '', $variations = array()) {
    // If this is a variable product with no variation selected, don't show the alert
    // We'll handle this with our custom scroll behavior instead
    if (!$passed && empty($variation_id) && !empty($variations)) {
        // Check if we're on a product page (not AJAX request)
        if (!wp_doing_ajax() && is_product()) {
            // Return true to bypass the validation, we'll handle it with JS
            return true;
        }
    }
    return $passed;
...
/**
 * Custom YITH Gift Card text translations
 */
add_filter( 'gettext_with_context', 'custom_yith_gift_card_texts_context', 20, 4 );
function custom_yith_gift_card_texts_context( $translation, $text, $context, $domain ) {

    if ( in_array( $domain, array( 'yith-woocommerce-gift-cards', 'yith-woocommerce-gift-cards-premium' ) ) ) {
        switch ( $text ) {
            case 'Send Yourself':
                return 'Send to yourself';
            case 'Send to others':
                return 'Send to a mate';
            case 'Name':
                return 'Recipient\'s Name';
            case 'Message':
                return 'Message (optional)';
            case 'Your name':
                return 'From (your name)';
        }
    }
...
/**
 * Add custom ID to WooCommerce variation form
 */
add_filter( 'woocommerce_variation_form_args', 'p2c_add_id_to_variation_form' );
function p2c_add_id_to_variation_form( $args ) {
    $args['form_id'] = 'p2c-variation-form';
    return $args;
}

/**
 * AJAX handler to check if product is in 5-for-99 category
 */
function p2c_check_product_category() {
    if (!isset($_POST['product_id']) || !isset($_POST['category_slug'])) {
    ...
    $is_in_category = has_term($category->term_id, 'product_cat', $product_id);
...
add_action('wp_ajax_p2c_check_product_category', 'p2c_check_product_category');
add_action('wp_ajax_nopriv_p2c_check_product_category', 'p2c_check_product_category');

/**
 * AJAX handler to count items from 5-for-99 category in cart
 */
function p2c_count_category_items_in_cart() {
    if (!isset($_POST['category_slug'])) {
        wp_send_json_error('Missing category slug');
    }
    ...
    if (WC()->cart && !WC()->cart->is_empty()) {
        foreach (WC()->cart->get_cart() as $cart_item) {
            if (has_term($category->term_id, 'product_cat', $cart_item['product_id'])) {
...
add_action('wp_ajax_p2c_count_category_items_in_cart', 'p2c_count_category_items_in_cart');
add_action('wp_ajax_nopriv_p2c_count_category_items_in_cart', 'p2c_count_category_items_in_cart');


/**
 * Show stock status message after product title
 */
add_action('woocommerce_single_product_summary', 'show_stock_status_message', 6);

function show_stock_status_message() {
    global $product;

    if (!$product) return;

    $not_for_sale_category = 'not-for-sale';

    // Product not for sale
    if (has_term($not_for_sale_category, 'product_cat', $product->get_id())) {
        echo '<p style="color:orange; font-weight:bold; margin-top:10px;">This product is not for sale</p>';
        remove_action('woocommerce_single_product_summary', 'woocommerce_template_single_add_to_cart', 30);
    }
    // Product out of stock
    elseif (!$product->is_in_stock()) {
        echo '<p style="color:red; font-weight:bold; margin-top:10px;">This product is out of stock</p>';
    }
}

/**
 * Add back in stock notification button for out-of-stock products
 */
add_action('woocommerce_single_product_summary', 'add_back_in_stock_button', 7);

function add_back_in_stock_button() {
    global $product;
    ...
    // Add button to open Fancybox
    echo '<a href="#custom-lightbox-message" data-fancybox class="button" style="margin-top:10px;">Let me know when it\'s back in stock</a>';

    // Hidden div with the message
    echo '<div style="display:none;" id="custom-lightbox-message">';
    echo '<p style="font-size:16px; color:#000;">Enter your email and we will notify you when this product is back in stock.</p>';
    echo '<div class="klaviyo-form-XKY6iJ"></div>';
    echo '<div class="mcwidget-embed" style="margin-top:20px;" data-widget-id="14744927"></div>';
    echo '</div>';
}

// Initialize variation tooltip behavior for desktop (no scroll)
add_action('wp_footer', 'initialize_variation_tooltip_behavior', 90);

function initialize_variation_tooltip_behavior() {
    if (!is_product()) {
        return;
    }
    ?>
    ...
    jQuery('form.single_add_to_cart_button').on('click', function (e) {
        ...
        var sizeSelect = form.find('.variations select').first();

        if (!sizeSelect.val()) {
            e.preventDefault();

            // Remove existing tooltip
            form.find('.size-tooltip').remove();

            // Create tooltip
            var tooltip = jQuery('<div class="size-tooltip">Please select a size to proceed</div>');
            sizeSelect.closest('td, .value').append(tooltip);
...
// Initialize Fancybox for back in stock buttons (only if available)
add_action('wp_footer', 'initialize_back_in_stock_fancybox', 99);

function initialize_back_in_stock_fancybox() {
    if (!is_product()) {
        return;
    }

    // Only initialize if Fancybox is available
    if (wp_script_is('p2c-fancy-js', 'enqueued')) {
        ?>
        <script type="text/javascript">
        jQuery(document).ready(function($) {
            // Initialize Fancybox for back in stock buttons
            $('a[href="#custom-lightbox-message"]').fancybox({
                touch: false,
                hash: false,
                loop: false,
                infobar: false,
...
//17 nov custom code
/* Add product features under price */
add_action( 'woocommerce_single_product_summary', 'plus2_product_features_box', 12 );
function plus2_product_features_box() {
    ?>

    <!-- TOP MESSAGE BOX -->
    <div class="p2c-box">
        <span class="p2c-icon">park</span>
        <p>Every order supports reforestation by planting two trees with Our Forest.</p>
    </div>

    <!-- FEATURES LIST -->
    <div style="margin-top:15px;">
        <div class="p2c-item">
            <span class="p2c-icon-sm">check_circle</span>
            <p>Designed by Tall Aussies. Longline and Slim fit.</p>
        </div>
        ...
        <div class="p2c-item">
            <span class="p2c-icon-sm">check_circle</span>
            <p>Model is 6’1 (186cm) Wearing Large Size.</p>
        </div>
    </div>
...
// Stripe Connect Split functionality has been moved to a dedicated plugin
// The plugin "Stripe Connect Split for WooCommerce" now handles all Stripe Connect split payments
// with dynamic configuration via the WordPress admin interface.
