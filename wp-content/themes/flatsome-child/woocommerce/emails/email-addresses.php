...
/* CUSTOM WORK */
			...
			$prefix = esc_attr(get_option('customer_append_string'));
			$cust_name = str_replace($prefix, '', $name); ?>
			<address class="address">
			    <?php if ( $name ) : ?>
				<?php echo esc_html( $cust_name. ' '. $order->get_billing_last_name() ); ?>
				<?php endif; ?>
				<?php if ( $order->get_billing_address_1() ) : ?>
				<br/><?php echo esc_html( $order->get_billing_address_1()); ?>
				<?php endif; ?>
				<?php if ( $order->get_billing_address_2() ) : ?>
				<br/><?php echo esc_html( $order->get_billing_address_2()); ?>
				<?php endif; ?>
				<?php if ( $order->get_billing_city() ) : ?>
				<br/><?php echo esc_html( $order->get_billing_city()); ?>
				<?php endif; ?>
				<?php if ( $order->get_billing_postcode() ) : 
				$states = WC()->countries->get_states( $order->get_shipping_country() );
				?>
				<br/><?php echo esc_html( $states[$order->get_billing_state()].' '. $order->get_billing_postcode()); ?>
				<?php endif; ?>
				<?php if ( $order->get_billing_country() ) : ?>
				<br/><?php echo esc_html( WC()->countries->countries[$order->get_billing_country()]); ?>
				<?php endif; ?>
				<?php 
